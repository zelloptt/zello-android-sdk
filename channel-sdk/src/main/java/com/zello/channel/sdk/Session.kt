...
class Session internal constructor(
	...
	private fun reconnectWithRefreshToken(): Boolean {
		...
		if (refreshToken != null) {
			val shouldReconnect = sessionListener?.onSessionWillReconnect(this, ReconnectReason.UNKNOWN)
								  ?: false
			if (shouldReconnect) {
				performDisconnect()
				state = SessionState.CONNECTING
	...
	private fun handleLocationMessage(json: JSONObject) {
		...
		val address = if (json.has(Command.keyFormattedAddress)) {
			json.getString(Command.keyFormattedAddress)
		} else {
			null
		}
