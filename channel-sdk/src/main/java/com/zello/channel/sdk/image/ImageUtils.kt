...
internal object ImageUtils {
	...
	fun bitmapWithMaxDimensions(bitmap: Bitmap, maxWidth: Int, maxHeight: Int): Bitmap {
		if (bitmap.width <= maxWidth && bitmap.height <= maxHeight) {
			return bitmap
		}
		val targetDimensions = dimensionsFitting(bitmap.dimensions, Dimensions(maxWidth, maxHeight))
		return scaleBitmap(bitmap, targetDimensions.width, targetDimensions.height)
	}
	 ...
	 * @param maxSize the preferred maximum data length in bytes
	 * @return a ByteArray containing the compressed image data
	 */
	fun compressImage(image: Bitmap, maxSize: Int): ByteArray {
		var dataLength: Int
		var quality = 90
		val minQuality = 10
		...
		} while (dataLength > maxSize && quality >= minQuality)
		return stream.toByteArray() // Stream guaranteed to be non-null
	}

	private fun dimensionsFitting(original: Dimensions, max: Dimensions): Dimensions {
